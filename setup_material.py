...
def run_cif2cell_and_extract(cif_path, work_dir):
    ...
    print(f"  Detected lattice parameter A = {a_value:.6f} Å")
...
PROJECTION_ORBITALS = {
    "d_metal": ["s", "p", "d"],   # 1+3+5 = 9 orbitals × 2 (spinor) = 18 per atom
    "main":    ["s", "p"],        # 1+3   = 4 orbitals × 2 (spinor) =  8 per atom
}
...
def count_wann_per_atom(el):
    """Return the number of Wannier functions per atom (×2 for SOC spinors)."""
...
def estimate_wannier_params(all_atoms_in_cell, nbnd_input):
    ...
    nbnd     = max(num_wann × 1.5, nbnd_input) rounded up to a multiple of 4.

    Using num_wann × 1.5 (rather than num_wann + 4) ensures that the number
    ...
    nbnd_input : int
        Lower bound on the number of bands from nelec × 1.2.
    ...
    # nbnd >= num_wann × 1.5 and >= nbnd_input, rounded up to multiple of 4
...
# =======================================================================
# 7. k-point mesh generation
# =======================================================================

def generate_kpoints(n):
    """Uniform n×n×n k-point mesh (fractional coords + weights)."""
    w = 1.0 / n**3
    lines = []
    for x in range(n):
        for y in range(n):
...
def estimate_nelec(all_atoms_in_cell):
    ...
    that arose from the old unique_elements × (nat / len(elements)) approach.
    """
...
def main(formula: str):
    elements = parse_formula(formula)          # parse chemical formula into element list
    ...
    for d in [scf_dir, nscf_dir, wan_dir]:
        os.makedirs(d, exist_ok=True)

    # ------------------------------------------------------------------
    # Step A: Fetch CIF
    # ------------------------------------------------------------------
    print(f"\n[1/4] Fetching CIF for {formula} from AFLOW...")
    ...
    # ------------------------------------------------------------------
    print(f"\n[2/4] Converting CIF to QE format via cif2cell...")
    ...
    # ------------------------------------------------------------------
    print(f"\n[3/4] Resolving pseudopotentials...")
    ...
    # num_wann is derived from all_atoms_in_cell × projected orbitals,
    ...
    print(f"  num_wann={num_wann}  (= sum of atoms × orbitals × 2 spinor)")
    ...
    # ------------------------------------------------------------------
    # Step F: Write input files
    # ------------------------------------------------------------------
    print(f"\n[4/4] Writing input files...")
    ...
    print(f"\n--- Consistency Check ---")
    print(f"  projections  → {num_wann} Wannier functions  (= sum of atoms × orbitals × 2 spinor)")
    print(f"  num_wann     = {num_wann}")
    print(f"  num_bands    = {nbnd}  (margin: +{margin} bands above num_wann)")
    print(f"  dis_win_max  = {ewin['dis_win_max']} eV  [conservative initial value; adjust after SCF]")
    print(f"  dis_froz_max = {ewin['dis_froz_max']} eV  [set to E_Fermi + 2–5 eV after SCF]")
    print(f"\n⚠️  Post-Wannierization checklist:")
    print(f"  1. After SCF: grep 'Fermi' calc/{formula}/01_scf/scf.out")
    print(f"     → Obtain the Fermi energy (E_F)")
    print(f"  2. Set dis_froz_max = E_F + 2–5 eV in {seedname}.win")
    print(f"  3. Set dis_win_max below the energy of band #{nbnd} at all k-points")
    print(f"     (check 'Outer:' lines in {seedname}.wout; bands in window must be >= {num_wann})")
